...
class Clock extends React.Component {
  ...
  startTimer() {
    const newCurrentState =
      this.state.currentState === "session" ? "break" : "session";
  ...
  count(action, stateType) {
    const newStateTime =
      action === "-" ? this.state[stateType] - 1 : this.state[stateType] + 1;
  ...
  calc_dashoffset() {
    const currentTime =
      this.state.minutes +
      "." +
      this.pad(Math.round((this.state.seconds * 100) / 60));
      ...
      <div id="main-container">
        <div className="out">
          ...
          >
            <defs>
              <linearGradient id="GradientColor">
                <stop offset="0%" stopColor="#518c8f" />
          ...
          <div className="in">
            <div id="time-container" className="container">
              <p id="timer-label" className="text">
                {this.state.currentState}
              </p>
              <p id="time-left" className="text">
                {this.pad(this.state.minutes)}:{this.pad(this.state.seconds)}
              </p>
              <div className="btn_section">
                {Button(
                  this.btnStartPauseValue(),
                  "start_stop",
                  () => this.btnStartPause(),
                  "element"
                )}
                ...
                <audio
                  ...
                  disableRemotePlayback={!!this.state.timerId}
                ></audio>
              </div>
            </div>
          </div>
        ...
        <div className="btn_section">
          <div id="break-container" className="small_container">
            <p id="break-label" className="text">
              break length
            </p>
            <div className="btn_section">
              {Button(
                ...
                () => this.count("+", "break"),
                "element",
                !!this.state.timerId || this.state.break >= 60
              )}
              <p id="break-length" className="element text">
                {this.state.break}
              </p>
              {Button(
                ...
                () => this.count("-", "break"),
                "element",
                !!this.state.timerId || this.state.break <= 1
              )}
            </div>
          </div>
          <div id="session-container" className="small_container">
            <p id="session-label" className="text">
              session length
            </p>
            <div className="btn_section">
              {Button(
                ...
                () => this.count("+", "session"),
                "element",
                !!this.state.timerId || this.state.session >= 60
              )}
              <p id="session-length" className="element text">
                {this.state.session}
              </p>
              {Button(
                <FontAwesomeIcon icon={faMinus} size="lg" />,
                ...
                () => this.count("-", "session"),
                "element",
                !!this.state.timerId || this.state.session <= 1
              )}
            </div>
          </div>
        </div>
...
root.render(
  <BrowserRouter basename="/react-pomodoro">
    <Clock />
  </BrowserRouter>
);
